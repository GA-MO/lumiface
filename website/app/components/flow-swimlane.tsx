 ...
 *  docs' CSS variables so it reads in light and dark; the numbers match the steps on the backend page. */
        ...
        >
          <defs>
            {(["ink", "key", "weak"] as const).map((k) => (
              <marker key={k} id={`sw-${k}`} viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto-start-reverse">
          ...
          {ARROWS.map((a) => {
            ...
            return (
              <g key={a.n}>
                <circle cx="60" cy={a.y} r="10" fill="var(--color-fd-card)" stroke="var(--color-fd-border)" />
                <text x="60" y={a.y + 4} textAnchor="middle" fontSize="11" fontWeight="500" fontFamily="ui-monospace, monospace" fill="currentColor">{a.n}</text>
                ...
                <text x={mid} y={a.y - 10} textAnchor="middle" fontSize="11.5" fontFamily="ui-monospace, monospace" fill="currentColor">{a.label}</text>
                {a.note && <text x={mid} y={a.y + 16} textAnchor="middle" fontSize="11" fill="var(--color-fd-muted-foreground)">{a.note}</text>}
              </g>
            );
          })}
          ...
          {NOTES.map((n) => {
            ...
            return (
              <g key={n.n}>
                <circle cx="60" cy={n.y} r="10" fill="var(--color-fd-card)" stroke="var(--color-fd-border)" />
                <text x="60" y={n.y + 4} textAnchor="middle" fontSize="11" fontWeight="500" fontFamily="ui-monospace, monospace" fill="currentColor">{n.n}</text>

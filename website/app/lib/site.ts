const PLACEHOLDER_SITE_URL = "https://facegate.example.com";

function resolveSiteUrl() {
  const configured = process.env.FACEGATE_SITE_URL?.trim();
  return (configured && configured.length > 0 ? configured : PLACEHOLDER_SITE_URL).replace(/\/+$/, "");
}

/** Origin used by llms.txt, llms-full.txt and sitemap.xml. Set `FACEGATE_SITE_URL` in the Pages workflow. */

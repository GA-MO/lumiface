...
### Phase 13 – ไม่มี enroll บน server: `reference_photo` ต่อ session (ทำแล้ว 2026-09-19)
- เหตุผล: ลูกค้ามีระบบ user + รูปอยู่แล้ว ไม่อยากมี biometric ค้างที่ Lumiface (แบบ AWS Rekognition: CreateFaceLivenessSession + CompareFaces กับรูปของตัวเอง) — ตัดสินใจตัด enroll ออกจาก product ทั้งก้อน ไม่ใช่แค่ปิดได้
- **server**: `POST /v1/sessions {reference_photo: b64, purpose}` → `services/verify.reference()` (1 หน้า, ขนาด ≥ `min_face_size`, หน้าตรงตาม `reference_max_yaw/pitch` — เปลี่ยนชื่อจาก `enroll_max_*`; **ไม่** anti-spoof เพราะมาจาก backend ด้วย key) → embedding บน `verifysession.reference_embedding`, ลบใน statement เดียวกับ `used=True` ตอน device เปิด stream; `reference: bool` บน session/verification/`GET /verifications` (แทน `subject_id`); ลบ `Subject`, `EnrolToken`, `routers/subjects.py`, `subject_ttl_seconds`, `enrol_token_ttl_seconds`; `init_db` DROP TABLE `subject`, `enroltoken` ของ DB เก่า; retention เหลือ purge session; project delete ใช้ bulk delete (ORM ไม่มี relationship ให้เรียงลำดับ — เจอ FK fail ตอนตัด subject ออก)
- **STORE_FRAMES** เก็บ `reference.npy` ต่อ session (dev setting ที่เก็บทุกเฟรมอยู่แล้ว) เพื่อให้ replay match ได้; `replay_sessions.py`: `reference_embedding(meta, --reference, folder)` = `reference.npy` → `--reference photo` → `subjects/<id>.npy` ของ session เก่าที่มี `subject_id`; `attack_sessions.py --reference photo`
- **SDK**: Flutter 0.4.0 / React 0.3.0 — ลบ `FaceFlow.enroll`, `FaceEnrollController`, `enrolTokenProvider`, `LumifaceClient.enroll`, `Subject`, `FrameCapturer`/`captureJpeg`, `image_convert.dart`+`rawFrameToJpeg`, glue JS `captureJpeg`; `FaceFlowController` ยุบเข้า `FaceVerifyController`; `sessionProvider` required; Flutter ไม่ต้องใช้ `dio`/`image` แล้ว; `VerificationRecord.subjectId` → `reference`
- **examples/backend** เป็น user store ในหน่วยความจำ (`POST/GET/DELETE /api/users` + รูป) และ `POST /api/face/session {user_id?}` อ่านรูปของ user นั้นส่งเป็น `reference_photo`; Flutter tab Subjects → Users (ถ่าย selfie/เลือกรูป → backend ตัวอย่าง ไม่ใช่ Lumiface); React มี form register
- docs อ่านทั้งหมดเทียบ code: api/concepts/security(retention)/reason-codes/get-started/flutter×4/react/server×3/roadmap/protocol/README×2/CLAUDE.md
- **identity บนเฟรม flash** (ต่อเนื่อง): เฟรมกลางของทุก flash window เข้า match + consistency ด้วย (ไม่เข้า anti-spoof เพราะย้อมสี) → 6 จุด; พิสูจน์กับ replay set ก่อน: genuine 27 ผ่านหมด, match บนเฟรม flash 0.62–0.76 เทียบ key frame 0.63–0.81 (แย่สุด 0.6206 vs threshold 0.45), consistency 0.79–0.93 ไม่เปลี่ยน; test `test_face_swapped_in_for_the_flash_is_caught` (คน B โผล่เฉพาะช่วง flash → `NO_MATCH`, liveness → `INCONSISTENT`) ผ่าน `run_stream(flash_jpeg=)`
- **backend ตัวอย่างสำหรับ 3 บทบาท** (front-end / back-end / คนติดตั้ง): หน้า `docs/backend.mdx` (2 REST calls + `session_id → user` ฝั่งลูกค้า + checklist), index.mdx จัดตามบทบาท; `examples/backend-node` (Bun/TS ไม่มี framework, route เดียวกับ Python) + Python เพิ่ม `SESSIONS` และ `/api/face/done` ตอบ `{…, user_id, verified}`; รัน e2e จริง (uvicorn + bun + stream session ที่เก็บไว้): FLASH_ENFORCE ปกติ → `verified:false/FLASH_FAIL` (สี plan ใหม่ ≠ ที่อัดไว้ ถูกต้อง), FLASH_ENFORCE=0 → `verified:true` match 0.91
- เสร็จเมื่อ (ผ่าน): `uv run pytest -q` 105 passed (test_replay ทั้ง 27+10 ผ่านด้วย `subjects/me.npy` ของ session เก่า), `flutter analyze/test` 28 passed, `bun run typecheck`, `bun run test:react` 27 passed, `bun run check:docs`

## สถานะ (updated 2026-09-19, branch `aws-detectors`, Phase 13 uncommitted)
ทำแล้ว: **Phase 13** (ยังไม่ commit) — ตาม section ข้างบน; ไม่ต้องรันสดเพราะฝั่ง device ส่ง stream เดิมทุกอย่าง (เปลี่ยนแค่ตอนสร้าง session บน backend) · ก่อนหน้า:
...
ค้าง: (1) threshold ทุกตัว (flash locality 0.8, smile 1.08, blink 0.75, spoof floor 0.30) ยังมาจากคนเดียว — ต้องคนอื่น ≥3 + มือถือรุ่นกลาง เปิด `STORE_FRAMES=1 DEBUG=1` ทุกรอบแล้วจัดเข้า `data/sessions/` (2) CPU บน iOS ยังไม่วัด (Android ~195% ของ 800%) (3) fps มือถือ 6.3–7.7 ไม่ถึง 8 เพราะ encode Dart ~150 ms (4) ~~กล่อง demo สาธารณะ~~ ขึ้นแล้ว 2026-09-20 บน Fly sin: `lumiface-demo.fly.dev` (server, 2x/2 GB suspend-when-idle ตั้งแต่ 2026-09-21 — ปลุก 0.8 s แทนโหลดโมเดล 40 s, volume) + `lumiface-demo-api.fly.dev` (`website/demo-backend` ถือ key + rate-limit ต่อ IP), repo var `LUMIFACE_DEMO_URL` ตั้งแล้ว — Pages build ถัดไปจาก `main` จะใช้ server จริง; e2e จากเครื่องนี้ผ่าน (`OK` ~10 s หลัง end) (5) ~~`server/data/frames/1/`~~ เคลียร์แล้ว 2026-09-21: 5 session face_move ที่ยัง replay ผ่านย้ายเข้า `data/sessions/genuine/` (32 genuine), ที่เหลือ (ซ้ำ/โปรโตคอลเก่า smile-blink-turn/รอบล้ม) ลบ (6) Local: team Apple ใน `examples/flutter/ios/Flutter/Local.xcconfig` (git-ignored), ติดตั้ง iPhone ด้วย `devicectl` เพราะ launcher Flutter ล้ม

...
- `packages/facegate-react`: port ของ controller/detectors เป็น TypeScript, `MediaPipeSource`, `FacegateView` + `useFacegate` render props เหมือน Flutter builders, vitest port ของ Dart tests
...
## สถานะ (updated 2026-09-18)
ทำแล้ว (2026-09-18, uncommitted): **เปลี่ยนชื่อเป็น Facegate** (package `packages/facegate`, server `facegate-server`, API employees→subjects, checkins→verifications, `checkin_id`→`verification_id`) · **SaaS policy**: `server/app/policy.py` – `Policy` (server fields + `client` = LivenessConfig) resolve ต่อ request = env → preset (`balanced`/`strict`/`relaxed`/`emulator`) → overrides ของ project, เก็บใน `Project.preset`/`policy_overrides`, services เรียก `get_policy()` ผ่าน `PolicyScopeMiddleware` (contextvar ต่อ request เพราะ FastAPI รัน sync dependency ใน threadpool คนละ context), endpoints `GET/PUT/DELETE /v1/policy`, `/v1/policy/presets`, `/v1/policy/schema`, admin `POST/GET/DELETE /v1/projects`, `/rotate-key` (`ADMIN_API_KEY`), session ตอบ `client_config` + `mode` (`verify`/`liveness` เมื่อไม่ส่ง subject_id) + `purpose`, CORS เปิด · pytest 53 ผ่าน · **Flutter**: `FaceVerifyView` (flow verify/liveness/enroll, `FaceVerifyTheme`, builder slots, `overlayBuilder`, `sourceFactory`), headless `FaceVerifyController` (config จาก session ถ้าไม่ส่ง) + `FaceEnrollController`, `LivenessConfig.fromJson/toJson`, `CameraFaceSource` แยกจาก package camera (buildPreview/setExposureLocked), example มี 5 use cases + preset picker · flutter test 39 ผ่าน · **Phase 5 web ทำแล้ว**: `MediaPipeCameraSource` (getUserMedia + FaceLandmarker ผ่าน glue `assets/facegate_mediapipe.js` โหลด tasks-vision@1.0.1 จาก jsDelivr; 0.10.x หายจาก CDN แล้ว) ทดสอบสดบน Chrome + Mac webcam: liveness flow ผ่านครบ (relaxed preset) flash corr 0.994 response 2.05 bg ratio 0.04 smile gain 1.24 CVPR 0.998 · **Phase 6 React SDK** `packages/facegate-react` (`@facegate/react`): client, controller port (vitest 21 ผ่าน, port ของ Dart tests), `MediaPipeSource`, `FacegateView`, `useFacegate`, demo Vite :3010 · **docs site** `website/` (fumadocs + react-router เหมือน agentic-ui, `policy-reference.mdx` generate จาก `policy_schema()`, llms.txt, GitHub Pages workflow) · root bun workspace + `.github/workflows/{pages,ci}.yml`
ค้าง: (0) ยังไม่ได้เปลี่ยนชื่อโฟลเดอร์ repo/remote เป็น `facegate` (ผู้ใช้ตัดสินใจ) (1) iPhone จริงยังไม่รันหลัง rename; MediaPipe `yawSign` เช็คจาก webcam เดียว (2) flash response บน laptop ต่ำ (2.05 vs min 2.0) → web project ควรใช้ relaxed หรือลด `flash_min_response` (3) Safari iOS ของ Flutter web ยังไม่ทดสอบ (4) React demo ยังไม่ได้รันสด (build/typecheck/test ผ่าน) (5) ข้อค้างเดิมด้านล่างยังอยู่

### สถานะเดิม (2026-09-17)
ทำแล้ว: (uncommitted 2026-09-18 02:10) ทดสอบสดบน Galaxy S25+ (SM-S936B, Android 16, ต่อ USB + adb reverse 8010): คนจริง 9 รอบ, replay จากมือถืออีกเครื่อง 2 รอบ → flash correlation คนจริง 0.95–0.99 response 11–19, replay 0.77–0.83 response 6–11 (**จอมือถือสะท้อน flash ได้** สมมติฐาน "จอไม่สะท้อน" ผิด) → เพิ่มกฎ locality: response รอบหน้า/บนหน้า ≤ 0.8 (จริง 0.47–0.52 ทั้ง 9, replay 1.15/2.31) แล้วเปิด `FLASH_ENFORCE=1` เป็นค่าเริ่มต้น · แก้บั๊ก 2 จุดจากการทดสอบ: neutral_end ถ่ายเร็วเกินหลัง flash (ยังติดสี MiniFASNet ตก 0.2) → `settleAfterFlashMs` 800; เฟรมพยักหน้า −40° consistency 0.47–0.55 → เกณฑ์ 0.40 สำหรับเฟรมเอียง >20° · emulator: คนจริง 3/3 ผ่าน, วิดีโอมือถือ 3/3 กันได้, รูปนิ่งเอียง 2/2 ค้าง TIMEOUT ฝั่งแอป (parallax) · pytest 47 ผ่าน · (2026-09-18 00:20) server ตรวจยิ้มซ้ำ + จอสว่างสุดตอน flash · (2026-09-17 22:05) screen-flash client+server, flutter test 24 ผ่าน, APK build ผ่าน (ยังไม่ได้รันสดเพราะไม่มีคนหน้ากล้อง) · ชุดทดสอบภายนอก AxonData (HF, CC BY-NC) 1.5GB ใน server/data/samples/axondata + `scripts/eval_videos.py`: genuine 14/14 ผ่าน, replay จอ/มือถือ 0/12, cut-out 0/5, paper mask 0/5 ผ่าน (กันได้หมด), **latex 3/3 silicone 2/3 ผ่าน** (กันไม่ได้ด้วย passive) · (uncommitted 2026-09-17 21:25) Phase 4 ขั้น B parallax: models/mlkit_camera_source/config/challenge_detector/liveness_controller + debug bar แสดง px= · flutter test 22 ผ่าน · วัดจริงด้วย ML Kit บน emulator จาก sample: หน้าจริงหัน 14°→26° Δpx=0.11, วิดีโอคนจริงหัน 0°→50° Δpx=0.59, รูปแบนบีบแนวนอน (ML Kit ยังรายงาน yaw −25) Δpx=0.026 → threshold 0.08 แยกได้ แต่ยังไม่ได้ลองคนจริงสดหน้ากล้อง · commit 4f019a6 = Phase 0–2 + Phase 3 ฝั่ง Android + Phase 4 ขั้น A · server: FastAPI + buffalo_l + 2 anti-spoof gate (MiniFASNet, CVPR-2024 ResNet50 face crop margin 0.3) + timing rules, pytest 32 ผ่าน · Flutter package `face_checkin` + example, flutter test 16 ผ่าน, iOS build ผ่าน · ทดสอบจริงบน Android emulator + webcam Mac (README มีวิธี): คนจริงผ่าน 3/3, รูปนิ่งกันได้ 4/4, วิดีโอบนจอกันได้ 6/6 หลังใส่ CVPR gate (ก่อนหน้า #10 เกือบหลุด) · ชุดข้อมูลถาวร server/data/samples/ (ไม่เข้า git) + fixtures ใน tests/fixtures (มีหน้าผู้ใช้ 9 ภาพ)

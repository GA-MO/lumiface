...
ทำแล้ว: (2026-09-18 21:20, uncommitted) **iOS native บน iPhone 12 ผ่าน**: หลัง Apple ID ใน Xcode (team ใน `ios/Flutter/Local.xcconfig` git-ignored, Podfile pin deployment target 15.5 เพราะ Xcode 26 ไม่รับ <15, ติดตั้งด้วย `devicectl` เพราะ launcher ของ Flutter ล้ม) เจอบั๊ก iOS-only 3 จุดใน `mlkit_camera_source.dart`: (1) `yawSign` กลับทิศ — camera_avfoundation ส่งเฟรมกล้องหน้ากลับด้าน (server ยืนยัน: turn_left ได้ yaw −47.7) (2) `captureJpeg` หมุนเฟรมซ้ำ 90° — iOS ส่งเฟรมตั้งตรงอยู่แล้ว → server ได้หน้านอนตะแคง → MiniFASNet 0.22–0.5 (SPOOF), match 0.17–0.5 (NO_MATCH) ทั้งที่ CVPR/flash ผ่าน; ตอนนี้ไม่หมุน + un-mirror (3) `isMirrored` false บน iOS · เพิ่ม: encode เฟรมเดียวกับที่ signal มาจาก (`_signalImage`) ไม่ใช่เฟรมล่าสุด — blink บน iOS ผ่าน 2/2 หลังแก้ · **ผลสุดท้าย iPhone 12 native: คนจริง 3/3 OK** (spoof 0.91–0.999, match 0.69–0.81, cvpr 1.0, flash resp 17–27 locality 0.37–0.47, fps 7.6–7.8) **replay 0/4** (TIMING_TOO_SLOW 3, FLASH_FAIL corr 0.05 resp 0.49 locality 7.26) · override `spoof_hard_floor 0.2` ที่ใส่ชั่วคราวถอดแล้ว — ค่า 0.30 ไม่ใช่ปัญหาเมื่อเฟรมถูกทิศ · gate: flutter 42 ผ่าน, check:docs ผ่าน
· **Android re-test หลังแก้ iOS** (S25+ release, 21:25): คนจริง 4/5 OK (match 0.69–0.79, spoof 0.90–0.999, flash resp 42–56 locality 0.36–0.42, fps 6.3–7.3, CPU เฉลี่ย 193% สูงสุด 259%) — `_signalImage` ไม่กระทบ; 1 รอบตก blink: เฟรมตาปิดมาถึงแล้วแต่ server EAR 0.16/0.22 = 0.727 > closed_ratio 0.70 (รอบผ่านได้ 0.65–0.67, รอบตกวันนี้ 0.71/0.727) → blink ของคนนี้ผ่าน ~6/9 ทุกแพลตฟอร์ม เป็นเรื่อง threshold ที่ขอบ ไม่ใช่ pipeline · gradle transforms cache หายเหมือน pub-cache (กู้ด้วยลบ transforms) น่าจะมาจากโปรแกรมล้างพื้นที่
· (2026-09-18 21:50, uncommitted) **replay harness** ตามที่ผู้ใช้ชี้: `_store_frames` เขียน `session.json` (plan, recv+client ms ทุกเฟรม/event, client, verdict), `scripts/replay_sessions.py` รัน `analyze_stream` ซ้ำบน session ที่เก็บไว้ (เทียบ verdict ตอนสด, override policy ผ่าน env), `tests/test_replay.py` ใน gate: `data/sessions/genuine/` ต้องผ่าน, `attack/` ต้องถูกปฏิเสธ ภายใต้ balanced (embedding ใน `data/sessions/subjects/me.npy`) — ชุดเริ่มต้น genuine 3 (iPhone native) attack 4 (วิดีโอ) ผ่าน 7/7; 13 session ที่เก็บวันนี้ backfill session.json จาก DB (ไม่มี client ms → จัดหน้าต่างด้วย server clock) replay ตรงกับตอนสด 10/13 (3 ที่ต่างคือรอบเฟรมตะแคง+override 0.2) · กฎใหม่ใน CLAUDE.md: ทดสอบสดทุกครั้งเปิด STORE_FRAMES=1 DEBUG=1 แล้วจัดเข้าชุด, แก้ server ต้องผ่าน replay ก่อนขอรอบสดใหม่ · ข้อจำกัด: ชุดนี้ทดสอบได้เฉพาะฝั่ง server — การแก้ SDK/กล้อง/เครื่องใหม่ยังต้องรอบสด
ค้าง: (1) /ship (1b) **ตัดสินใจ blink**: ลด `closed_ratio` 0.70→0.75 ใน `blink_observed` (วิดีโอ replay ที่เห็นวันนี้ได้ 0.85 ยังห่าง) หรือเอา blink ออกจาก pool เริ่มต้น — ยังไม่แก้ตามกฎไม่ปรับ threshold จากคนเดียว (2) CPU บน iOS ยังไม่วัด (3) `server/data/frames/1/` มีเฟรมทดสอบ 12 session จาก STORE_FRAMES (ใบหน้าผู้ใช้, ไม่เข้า git) ลบได้เมื่อไม่ต้องใช้ calibrate (4) เดิม: คนอื่น ≥3, มือถือรุ่นกลาง, server สาธารณะ, fps มือถือ ~7.5
ถัดไป: /ship → ทดสอบคนอื่น ≥3 + มือถือรุ่นกลาง → host demo server

### สถานะก่อนหน้า (2026-09-18 20:25)
ทำแล้ว: (2026-09-18 20:25, uncommitted) **iPhone 12 Safari ทั้ง 2 SDK ผ่าน** (React :3010 + Flutter web :3020 เสิร์ฟ https ใน LAN ด้วย self-signed CA, vite proxy `/v1`+wss และ `/api`, `DEMO_TLS_CERT/KEY` ใน `examples/react/vite.config.ts` + `cors: true` เพราะ Vite 6+ ตอบ preflight เอง): คนจริง 8/9 OK (blink ตก 1 ก่อนแก้), flash response 6.9–15.9 locality 0.39–0.52, fps 7.2–7.6, yaw ทิศถูก (turn_left +42.8/69.3, turn_right +47.6/63.4); **replay วิดีโอบน iPhone 0/5** (FLASH_FAIL resp 0.95 locality 3.04, TIMING_TOO_SLOW 2, EXPRESSION_MISMATCH 2) · **blink ที่ ~7 fps**: 3 รอบที่ตก server ไม่เคยเห็นเฟรมตาปิดสุด (EAR ต่ำสุด 0.71–0.85 ของ baseline, เกณฑ์ 0.70) → SDK ทั้งสองส่งเฟรมทันทีเมื่อ client เห็นตาปิด ข้ามตัวจำกัด fps (ไม่แตะ threshold) หลังแก้ blink ผ่าน #36 (29 เฟรมในหน้าต่าง, 0.66) · clientInfo เพิ่ม `sdk`, server เก็บ `user_agent` ของ WS ใน details · React example: stage responsive (≤680px เต็มจอ 3:4), debug overlay เป็น checkbox ปิดค่าเริ่มต้น · **iOS native ยังรันไม่ได้**: Simulator ไม่ได้เพราะ ML Kit pods ไม่มี arm64-simulator (Xcode 26 ตัด Rosetta) — RocketSim/SimCam ช่วยไม่ได้; iPhone จริง (GAMO, pair แล้ว) ติดแค่ signing = Apple ID ใน Xcode · **docs อ่านเต็มเทียบ code** เจอ stale 14 จุด แก้หมด + `bun run check:docs` ใน ship gate + CLAUDE.md กฎอ่าน docs เต็มเมื่อปิด phase · `lumiface` เป็น Android plugin เปล่าที่ถือ `consumer-rules.pro` (R8/ML Kit) แทน proguard ฝั่งแอป · gate: pytest 65, flutter 42, vitest 28, typecheck 0, check:docs ผ่าน, site build ผ่าน
ค้าง: (1) /ship ชุดนี้ (2) iOS native: ต้อง Apple ID ใน Xcode ครั้งเดียว แล้ว `DEVELOPMENT_TEAM` ใน pbxproj → `flutter run --release -d GAMO` (3) `~/.pub-cache` หายระหว่าง session (สาเหตุไม่ทราบ, กู้ด้วย pub get) (4) เดิม: server สาธารณะสำหรับ demo :3002, fps มือถือ ~6.5, encode native, คนอื่น/มือถือรุ่นอื่น
ตัดสินใจ (2026-09-18 20:40): **ไม่แทน ML Kit ด้วย MediaPipe native** — ปัญหา ML Kit ที่เจอแก้ได้แล้ว (R8 ที่แพ็กเกจ) หรือมีทางถูกกว่า (Simulator → เครื่องจริง + Apple ID) และไม่มี false reject ใดโทษ detector ฝั่ง client ได้ · ทบทวนเมื่อ (a) Flutter ทำให้ plugin ไม่มี SPM เป็น error หรือ (b) calibrate กับคนหลายคนพบว่าเว็บ/แอปต้องใช้ client threshold คนละชุด
ถัดไป: ทดสอบคนอื่น ≥3 + มือถือรุ่นกลาง → Apple ID → iOS native (yawSign ML Kit, CPU) → host demo server

### สถานะก่อนหน้า (2026-09-18 18:45)

...
The landing-page demo (`website/app/components/home/live-demo-runner.tsx`) runs the real SDK against the demo
server when the site is built with `VITE_LUMIFACE_DEMO_URL` (a `website/demo-backend` holding the demo key), and
against a stand-in that judges nothing otherwise; its copy must say which, and the demo box must run with
`STORE_FRAMES=0 DEBUG=0` because the copy promises no frame outlives the verdict.

...
from .services.inference import pool
...
def bootstrap_project() -> None:
    ...
    with Session(get_engine()) as db:
        ...
        db.add(Project(name=s.bootstrap_project_name, api_key_hash=hash_api_key(key), preset=preset))
        db.commit()
...
async def lifespan(app: FastAPI):
    init_db()
    bootstrap_project()
    # The models (~30 s on a small machine) load on the inference pool so the port answers at once: a session can be
    # created meanwhile, and the first stream to need them waits on the getters' locks.
    warmup = asyncio.get_running_loop().run_in_executor(pool(), lambda: (get_face_engine(), get_antispoof()))

...
def test_busy_server_refuses_the_socket_and_keeps_the_session(client, reference):
    """Past MAX_OPEN_STREAMS the hello is never read: the device gets SERVER_BUSY (1013) and the session is
    still unspent, so a retry against a quieter worker goes through."""
    from app.config import get_settings

    first = client.post("/v1/sessions", headers=HEADERS, json={"reference_photo": reference}).json()
    second = client.post("/v1/sessions", headers=HEADERS, json={"reference_photo": reference}).json()
    get_settings().max_open_streams = 1
    try:
        with client.websocket_connect(f"/v1/sessions/{first['session_id']}/stream") as ws:
            ws.send_json({"type": "hello", "token": first["session_token"]})
            assert ws.receive_json()["type"] == "plan"
            with client.websocket_connect(f"/v1/sessions/{second['session_id']}/stream") as busy:
                assert busy.receive_json() == {"type": "error", "reason_code": "SERVER_BUSY"}
        assert client.get(f"/v1/sessions/{second['session_id']}", headers=HEADERS).json()["used"] is False
    finally:
        get_settings().max_open_streams = 0
    with client.websocket_connect(f"/v1/sessions/{second['session_id']}/stream") as ws:
        ws.send_json({"type": "hello", "token": second["session_token"]})
        assert ws.receive_json()["type"] == "plan"
...
def test_flash_enforced_rejects_unlit_frames(client, person_crops, reference):
    ...
    assert d["gates"] == {"frames": "pass", "order": "pass", "timing": "pass", "static": "pass",
                          ...
                          "minifasnet": "pass", "cvpr2024": "pass" if get_settings().cvpr_enabled else "skipped",
                          "match": "pass", "consistency": "pass"}
    assert d["flash"]["enforced"] is True and len(d["match"]) == 6 and 0 < d["consistency"] <= 1
    assert all(0 <= f["spoof"] <= 1 and 0 <= f.get("cvpr", 0) <= 1 for f in d["key_frames"] if not f["kind"].startswith("flash"))

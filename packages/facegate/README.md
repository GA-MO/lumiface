...
Face verification with active liveness for Flutter (iOS, Android, web), backed by the Facegate server.

```dart
final client = FacegateClient(baseUrl: 'https://faces.example.com', apiKey: key);
...
- `FacegateClient`: sessions, verify, subjects, verifications, policy.
...
iOS: `NSCameraUsageDescription`, platform ≥ 15.5. Android: minSdk ≥ 23. Web: MediaPipe tasks-vision is loaded from jsDelivr by the package asset `assets/facegate_mediapipe.js`; pass `MediaPipeCameraSource(tasksVisionUrl:, modelUrl:)` through `sourceFactory` to self-host.

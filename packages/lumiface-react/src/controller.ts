...
export function alignHint(s: FaceSignal, config: LivenessConfig, region: Box = FULL_FRAME, maxWidth = config.maxFaceWidthFraction, minWidth = config.minFaceWidthFraction): AlignHint | null {
  if (s.faceCount === 0 || !s.box) return "noFace";
  if (s.faceCount > 1) return "multipleFaces";
  const b = boxInRegion(s.box, region);
  if (b.width < minWidth) return "tooFar";
...
export class FaceVerifyController {
  ...
  /** The face widths that count as aligned, in the visible region: the config's bounds, or, when the oval's
   *  start is narrower than the upper one (a landscape preview shrinks the oval), both scaled down together so
   *  the band keeps its proportion instead of collapsing to a sliver between "closer" and "back". */
  private alignBand(): { min: number; max: number } {
    const { minFaceWidthFraction: min, maxFaceWidthFraction: max } = this.config;
    const oval = this.ovalBox();
    if (!oval) return { min, max };
    const start = boxInRegion(oval, this.visibleRegion).width * this.config.moveStartMaxRatio;
    return start < max ? { min: min * (start / max), max: start } : { min, max };
  }

  private align(s: FaceSignal) {
    const band = this.alignBand();
    const hint = alignHint(s, this.config, this.visibleRegion, band.max, band.min);
    if (hint) {

import { FacegateClient } from "../src/client.ts";
...
export class FakeClient extends FacegateClient {
  ...
  override async enroll(options: { externalId: string; name?: string }): Promise<Subject> {
    ...
    if (options.externalId === "REJECT") {
      const { FacegateError } = await import("../src/types.ts");
      throw new FacegateError("POSE_NOT_FRONTAL");
    }

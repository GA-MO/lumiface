...
import { FacegateClient, FacegateView, TH, type FaceFlow, type FacegateScope, type VerifyResult } from "../src/index.ts";

type Mode = "checkin" | "login" | "liveness" | "enroll" | "custom";

function CustomOverlay({ scope }: { scope: FacegateScope }) {
...
function App() {
  ...
  const client = useMemo(() => new FacegateClient({ baseUrl, apiKey }), [baseUrl, apiKey]);
  ...
  return (
    <main>
      ...
      {mode === null ? (
        ...
        <div className="stage">
          {mode === "custom" ? (
            <FacegateView {...common} subjectId={subjectId || null} purpose="custom" renderOverlay={(scope) => <CustomOverlay scope={scope} />} />
          ) : mode === "login" ? (
            <FacegateView {...common} subjectId={subjectId} purpose="login" theme={{ guideShape: "roundedRect", guideActive: "#2dd4bf", guideSuccess: "#2dd4bf" }} />
          ) : (
            <FacegateView {...common} flow={flow} subjectId={subjectId || null} purpose={mode} replaceEnrollment />
          )}
        </div>
      )}

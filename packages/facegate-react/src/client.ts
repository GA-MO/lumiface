...
export interface FacegateClientOptions {
  baseUrl: string;
  apiKey: string;
  fetch?: typeof fetch;
}
...
export class FacegateClient {
  private readonly baseUrl: string;
  private readonly apiKey: string;
  private readonly fetchImpl: typeof fetch;

  constructor(options: FacegateClientOptions) {
  ...
  private async json<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (status >= 400) throw new FacegateError(reasonCode(body, status), body);
    return body as T;
  }

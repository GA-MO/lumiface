...
/// [maxWidth] and [minWidth] override [LivenessConfig.maxFaceWidthFraction] and
/// [LivenessConfig.minFaceWidthFraction]: the controller passes the oval's start width (and the lower
/// bound scaled with it) so the walk into the oval always begins from where the person held still,
/// whatever the frame's aspect.
AlignHint? alignHint(FaceSignal s, LivenessConfig config, [Rect region = fullFrame, double? maxWidth, double? minWidth]) {
  ...
  if (b.width < (minWidth ?? config.minFaceWidthFraction)) return AlignHint.tooFar;
...
class FaceVerifyController {
  ...
  /// The face widths that count as aligned, in the visible region: the config's bounds, or, when the
  /// oval's start is narrower than the upper one (a landscape preview shrinks the oval), both scaled
  /// down together so the band keeps its proportion instead of collapsing to a sliver between
  /// "closer" and "back".
  ({double min, double max}) get _alignBand {
    final min = config.minFaceWidthFraction;
    final max = config.maxFaceWidthFraction;
    final oval = _ovalRect;
    if (oval == null) return (min: min, max: max);
    final start = boxInRegion(oval, visibleRegion).width * config.moveStartMaxRatio;
    return start < max ? (min: min * (start / max), max: start) : (min: min, max: max);
  }

  void _align(FaceSignal s) {
    final band = _alignBand;
    final hint = alignHint(s, config, visibleRegion, band.max, band.min);
    if (hint != null) {
      _alignedSince = null;

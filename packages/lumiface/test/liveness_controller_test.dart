...
void main() {
  ...
  test('a full-width landscape preview keeps a band to stand in: the lower bound shrinks with the oval', () async {
    await boot([Challenge.faceMove], oval: const OvalTarget(cx: 0.5, cy: 0.45, width: 0.62, heightRatio: 1.35));
    c.frameAspect = 16 / 9;
    ...
    Rect at(double w) => Rect.fromCenter(center: const Offset(0.5, 0.45), width: w, height: 0.5);
    await emit(neutral(0).copyWith(box: at(0.3)));
    expect(c.state.value.hint, AlignHint.tooClose);
    await emit(neutral(100).copyWith(box: at(0.12)));
    expect(c.state.value.hint, AlignHint.tooFar);
    await emit(neutral(200).copyWith(box: at(0.22)));
    expect(c.state.value.hint, AlignHint.holdStill);
    await emit(neutral(900).copyWith(box: at(0.22)));
    expect(c.state.value.phase, LivenessPhase.challenge);
  });
